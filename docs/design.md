...
- **agent** — 各ノードに常駐するデーモン。イベント収集 → ルール評価 → アクション実行。**イベントストアを tail して、他プロセス（camera / mic）が追記した change イベントにもルールを発火させる（ストア＝イベントバス）**。
...
| **4** | メディアイベント源（マイク・カメラ） | ✅ v0 実装済（要実機検証） | カメラの動体検知（ffmpeg + フレーム差分）と、マイクのビープ検知（Goertzel。洗濯機・インターホン等の「ピー」を周波数プロファイルで拾う）がトリガーに加わる。生データはノードの外に出ず、スナップショットは MCP 経由の明示要求時のみ image content で返す。人物検知・汎用音分類（ML）は将来の増設（依存を持つ別パッケージとして） |
...
- **2026-07-07** — **Phase 4 v0 を実装**。方針: ML を持ち込まず、依存ゼロで届く範囲を先に取る。(1) カメラ＝ffmpeg（システムツール）で1本だけデバイスを開き、低解像度グレースケールのフレーム差分＋ヒステリシスで動体検知。同じパイプラインの第2出力（MJPEG, fd3）から最新1枚をメモリ保持し、明示要求時のみ `/snapshot`（→ MCP の image content）で返す。(2) マイク＝Goertzel アルゴリズム自作で家電のビープを周波数プロファイル検知（較正 CLI 同梱）＋大きな音の RMS 検知。(3) **イベントストアを正式にイベントバス化**: media デーモンはストアに追記するだけ、agent が tail して外部 change イベントにもルールを発火（`TSUKUMO_TAIL_SOURCES`）。カタログにも camera / mic が載り、kotodama がそのまま語彙にできる。人物検知・汎用音分類は「依存を持つ別パッケージ」として将来増設。
